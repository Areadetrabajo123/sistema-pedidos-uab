...
// 3. Renderizado de tabla principal (Orden: Producto, Cantidad, Unidad)
...
// 4. Panel de Pedido (Orden: Producto, Cantidad, Unidad)
function updateOrderPanel() {
  ...
  aggregatedData.forEach(item => {
    ...
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${item.product}</td><td class="num">${item.qty.toFixed(2)}</td><td>${item.unit}</td>`;
...
function renderFreqModal() {
  ...
  entries.forEach(e => {
    const tr = document.createElement('tr');
    tr.innerHTML = `
      <td>${e.name} <span class="freq-badge">Top ${Math.ceil((entries.indexOf(e)+1)/entries.length*10)}%</span></td>
      <td>${e.unit}</td>
      <td class="num">${e.count}</td>
      <td class="num">${e.totalQty.toFixed(2)}</td>
    `;
...
// 6. Exportación + Actualiza Historial
exportBtn.addEventListener('click', () => {
  ...
  aggregatedData.forEach(item => {
    const key = `${normalize(item.product)}|${normalize(item.unit)}`;
    if (!selectedKeys.has(key)) return;
    data.push({ Producto: item.product, Unidad: item.unit, Cantidad: item.qty });
  });
